  ...
  for(i in 2:nrow(df_table)){
    ...
    ifelse(pvalue<=0.05,
           if(str_detect(df_table[i,'Variable'], ":")){
             ...
             df_table[i,'Interpretation (5% error)'] <- paste0("The predictor ", df_table[i,'Variable'], " statistically significant. ")},
...
knitr::kable(df_table, digits=2, escape = T, linesep = '', caption=cat(" ", fill=TRUE), longtable = T) %>%
  kable_styling(position = "center", full_width = FALSE, latex_options = c("HOLD_position","repeat_header")) %>%
  footnote(general = "A significant interaction means the effect of one in the interaction involved predictor depends on (the levels of) the other predictor. ")  
