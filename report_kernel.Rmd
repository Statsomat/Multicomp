...
#Änderung von Erstellung von df_factorized in if-Schleife, damit keine leeren Dateien
#df_cat<-as.data.frame(df_cat)
df_cat_factor <- lapply(df_cat, as.factor)
df_cat_factor<-as.data.frame(df_cat_factor)
# 
# if (is.null(ncol(df_cat_factor))==TRUE) {
#       df_factorized <- df_cont
# } else if (is.null(ncol(df_cont))==TRUE) {
...
#tryCatch(error=function(e) {
  tryCatch( 
  if(eval2==FALSE){
  stop(safeError("TOO MANY MISSING VALUES"))
...
# } else {#Weiter nur mit vollständigem Datensatz ohne NAs
