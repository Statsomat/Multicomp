...
if (exists("df_cont")){
  ...
  if (ncol(df_cont)>0){
    ...
    cat("0", fill=TRUE)
    cat("\\newline",fill=TRUE)
    cat("Error: There are no continuous response variables of interest in the dataset. Execution stopped. ")
    eval <- FALSE
  }
} else {
  cat("Error: There are no continuous response variables of interest in the dataset. Execution stopped. ")
...
```{r falsechar, results="asis", eval=eval, warning = FALSE}
...
```{r Dunnet, eval = eval_model4, results='asis', dev="cairo_pdf", warning = TRUE}
...
```{r dunnet_stepdown, dev = "cairo_pdf", eval = eval_model4, results = "asis", warning = TRUE}
...
for (i in 1:nrow(glht_dunnet)) {
  if(glht_dunnet[i,]$P.value <= 0.05){
    ...
    glht_dunnet[i,]$sig <- paste0("Not Significant. Level ", lev[i+1], " of factor ", key , " is not ", sidetext, " than " , factorlevel, footnote_marker_number(2, "latex"))}
}
# footnote significance codes
names(glht_dunnet)[6] <- "Significance at 5 percent Type I error"
fn1 <- "'***': sign. to 0.1% error. '**': sign. to 1% error. '*': sign. to 5% error. ' . ': sign. to 10% error. '     ': not sign. ' - ': no statement."
...
```{r dunnet_sandwich, dev = "cairo_pdf", eval = eval_model4, results = "asis", warning = TRUE}
...
for (i in 1:nrow(sandw.df)) {
  if(sandw.df[i,]$P.value <= 0.05){
    ...
    sandw.df[i,]$sig <- paste0("Not Significant. Level ", lev[i+1], " of factor ", key , " is not ", sidetext, " than " , factorlevel, footnote_marker_number(3, "latex"))}
}
...
```{r DunnetStepdownSandwich, dev= "cairo_pdf", eval = eval_model4, results='asis', warning = TRUE}
...
for (i in 1:nrow(dunnet_sw_sd)) {
  if(dunnet_sw_sd[i,]$P.value <= 0.05){
    ...
    dunnet_sw_sd[i,]$sig <- paste0("Not Significant. Level ", lev[i+1], " of factor ", key , " is not ", sidetext, " than " , factorlevel, footnote_marker_number(2, "latex"))}
}
# footnote significance codes
...
```{r messagewarning, results="asis", eval = eval}
cat("# Warning", fill=TRUE)
cat("Please consider that this report depends on hard-coded cutoff-values which may be too liberal or too conservative for your research area. The automatic computation and interpretation delivered by the Statsomat/Multicomp app should not completely replace the classical, human-based graphical exploratory data analysis and statistical analysis. There may be data cases for which the Statsomat does not deliver the most optimal solution or output interpretation.")
```    
