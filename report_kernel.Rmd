...
cat("Note: No adjustement for multiplicity. Interpretation in the last column suitable only to categorical predictors. ", fill=TRUE)
...
knitr::kable(df_table, digits=2, escape = T, linesep = '', caption=cat(" ", fill=TRUE), longtable = T) %>%
  kable_styling(position = "center", full_width = FALSE, latex_options = c("HOLD_position","repeat_header")) %>%
  footnote(general = "If any, a significant interaction means the effect of one in the interaction involved predictor depends on (the levels of) the other predictor. ")  
...
GoF$Explanation <- c("Fraction of variance explained by the model.", 
                     "Adjusted R-Squared (penalizes the number of predictors).", 
...
tryCatch({
  ...
  if (bf$p.value <= alphahomog) {
     cat("Warning: Group variances significantly heterogeneous at 10% error! Results from The Analysis of Variance chapter may not be valid. ", fill=TRUE)
  } else {
...
}, error = function(e) {cat("Error: The Brown and Forsyth Test of Homogeneity of Variances cannot be executed. ")})
```
