  ...
  for (i in 2:nrow(coef.lmfit1)) {
    ...
    if(pvalue <= 0.05){
      if(str_detect(coef.lmfit1[i,]$Variable, ":")==T){
        coef.lmfit1[i,]$sig <- paste0("Interaction effect if levels ", sub(":", paste0(" and "), coef.lmfit1[i,]$Variable)," are present is significant. ")
                                
      }else coef.lmfit1[i,]$sig <- paste0("Significant difference between ", 
                                          coef.lmfit1[i,]$Variable, " and the reference of this factor. ")
    }else{ 
      if(str_detect(coef.lmfit1[i,]$Variable, ":")==T){
        coef.lmfit1[i,]$sig <- paste0("Interaction effect if levels ",sub(":", paste0(" and "), coef.lmfit1[i,]$Variable)," are present is not significant. ")
...
tryCatch({
  ...
  for(i in 2:nrow(df_table)){
    ...
    ifelse(pvalue<=0.05,
           if(str_detect(df_table[i,'Variable'], ":")){
             df_table[i,'Interpretation (5% error)'] <- paste0("Interaction ", df_table[i,'Variable'], " statistically significant. Effect of one involved factor depends on the other factor. ")
           } else {
  ...
  stop(safeError("ANOVA Type III table cannot be created. Execution stopped. "))
...
GoF$Explanation <- c("Fraction of variance explained by the model.", 
                     "Adjusted R-Squared by penalizing the number of predictors.", 
...
```{r levene, eval = eval_model4, results="asis"}
cat("### Levene Test of Homogeneity of Variances ", fill=TRUE)
```

```{r levene2, eval = eval_model4}
tryCatch({
