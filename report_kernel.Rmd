...
if (bf$p.value <= alphahomog) {
   cat("Warning: Group variances significantly heterogeneous at 10% type I error. Hypothesis tests from the Analysis of Variance chapter may not be valid. Moreover, we recommend in this case the consideration of the subsequent Dunnet test based on the Sandwich estimator. ", fill=TRUE)
} else {
   cat("Notice: We assume that the group variances are homogeneous. The subsequent Dunnet test shall be valid. ", fill=TRUE)
   homogtest <- TRUE
}
```

```{r browntest3, dev="cairo_pdf", eval = bftest, results="asis", fig.width=8, fig.cap="Homogeneity of Variance Plot"}
...
for (i in 1:nrow(glht_dunnet)) {
  if(glht_dunnet[i,]$P.value <= 0.05){
    glht_dunnet[i,]$sig <- paste0("Significant. Level ", lev[i+1], " of factor ", key, " is significantly ", sidetext, " from ", factorlevel, footnote_marker_number(3, "latex"))
  }else{ 
    glht_dunnet[i,]$sig <- paste0("Not Significant. Level ", factorlevel, " of factor ", key , " is not ", sidetext, " from " , lev[i+1], footnote_marker_number(2, "latex"))}
}
# footnote significance codes
names(glht_dunnet)[6] <- "Significance at 5 percent Type I error"
fn1 <- "'***': sign. to 0.1% error. '**': sign. to 1% error. '*': sign. to 5% error. ' . ': sign. to 10% error. '     ': not sign. ' - ': no statement."
...
for (i in 1:nrow(sandw.df)) {
  if(sandw.df[i,]$P.value <= 0.05){
    sandw.df[i,]$sig <- paste0("Significant. Level ", lev[i+1], " of factor ", key, " is significantly ", sidetext, " from ", factorlevel, footnote_marker_number(4, "latex"))
  }else{ 
    sandw.df[i,]$sig <- paste0("Not Significant. Level ", factorlevel, " of factor ", key , " is not ", sidetext, " from " , lev[i+1], footnote_marker_number(3, "latex"))}
}
...
for (i in 1:nrow(dunnet_sw_sd)) {
  if(dunnet_sw_sd[i,]$P.value <= 0.05){
    dunnet_sw_sd[i,]$sig <- paste0("Significant. Level ", lev[i+1], " of factor ", key, " is significantly ", sidetext, " from ", factorlevel, footnote_marker_number(3, "latex"))
  }else{ 
    dunnet_sw_sd[i,]$sig <- paste0("Not Significant. Level ", factorlevel, " of factor ", key , " is not ", sidetext, " from " , lev[i+1], footnote_marker_number(2, "latex"))}
}
# footnote significance codes
