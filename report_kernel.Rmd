...
if (length(which(complete_rate < 1)) != 0L){
  ...
  cat("Warning: There are missing values in the dataset. We will consider in what follows the analysis based only on the complete cases. ", fill=TRUE)
  }
```
    ...
    if(pvalue <= 0.05){
      if(str_detect(coef.lmfit1[i,]$Variable, ":")==T){
        coef.lmfit1[i,]$sig <- paste0("Interaction significant. Effect ",sub(":", paste0(" vs. ","reference ", "depends on "), coef.lmfit1[i,]$Variable),".")
                                          ...
                                          coef.lmfit1[i,]$Variable, " and the reference.")
    }else{ 
      if(str_detect(coef.lmfit1[i,]$Variable, ":")==T){
        coef.lmfit1[i,]$sig <- paste0("Interaction not significant. Effect ",sub(":", paste0(" vs. ","reference ", "doesn't depend on "), coef.lmfit1[i,]$Variable),".")
      }else coef.lmfit1[i,]$sig <- paste0("No significant difference between ",coef.lmfit1[i,]$Variable, " and the reference.")}
...
cat("Note: No adjustement for multiplicity.", fill=TRUE)
...
```{r effectcoding, eval = eval_model}
...
for(i in 2:nrow(df_table)){
  ...
  ifelse(pvalue<=0.05,
         if(str_detect(df_table[i,'Variable'], ":")){
           df_table[i,'Interpretation (5% error)'] <- paste0("Interaction ", df_table[i,'Variable'], " statistically significant.")
         } else {
           df_table[i,'Interpretation (5% error)'] <- paste0("There exist significant differences between the levels of factor ", df_table[i,'Variable'], ".")},
         
         if(str_detect(df_table[i,'Variable'], ":")){
           df_table[i,'Interpretation (5% error)'] <- paste0("Interaction ", df_table[i,'Variable'], " is not statistically significant.")
         } else {
           df_table[i,'Interpretation (5% error)'] <- paste0("There are no significant differences between the levels of factor ", df_table[i,'Variable'], ".")
         }
      )
}
...
```{r GoodnessOfFit, dev="cairo_pdf", eval = eval_model3, results='asis'}
...
```{r diagnosticplots, dev="cairo_pdf", eval=eval_model3, results="asis"}
...
```{r def_control_homog, eval = eval_model3}
# Define execution especially for a one-way model 
oneway <- as.logical(eval_model3*(length(df_cat_factor)==1L))
```


```{r titlevariances, dev="cairo_pdf", eval = eval_model3,  results="asis"}
...
```{r levene, eval = eval_model3, results="asis"}
cat("### Levene Test of Homogeneity of Variances ", fill=TRUE)
```

```{r levene2, eval = eval_model3}
tryCatch({
...
```{r Dunnet, eval=eval_model3, results='asis', dev="cairo_pdf"}
...
```{r dunnet_stepdown, dev = "cairo_pdf", eval = eval_model3, results = "asis"}
...
```{r dunnet_sandwich, dev = "cairo_pdf", eval = eval_model3, results = "asis"}
...
```{r DunnetStepdownSandwich, dev= "cairo_pdf", eval=eval_model3, results='asis'}
