...
if (length(which(complete_rate < 1)) != 0L){
  ...
  cat("Warning: There are missing values in the dataset. We will consider in what follows only the complete cases. ", fill=TRUE)
  }
```
...
tryCatch({
  ...
  e
  cat("Error: The factor of interest cannot be releveled. Execution stopped. ")
...
```{r Parametertable, dev="cairo_pdf", eval = eval_model, results='asis'}
...
```{r type3, dev="cairo_pdf", eval=eval_model, results='asis'}
# Type III tests 
...
```{r GoodnessOfFit, dev="cairo_pdf", eval = eval_model, results='asis'}
...
```{r diagnosticplots, dev="cairo_pdf", eval=eval_model, results="asis"}
...
```{r def_control_homog, eval = eval_model}
# Define execution especially for a one-way model 
oneway <- as.logical(eval_model*(length(df_cat_factor)==1L))
```


```{r titlevariances, dev="cairo_pdf", eval = eval_model,  results="asis"}
...
```{r levene0, eval = oneway, results="asis"}
cat("\\pagebreak")
``` 
 
```{r levene, eval = eval_model, results="asis"}
cat("### Levene Test of Homogeneity of Variances ", fill=TRUE)
```

```{r levene2, eval = eval_model}
tryCatch({
...
```{r Dunnet, eval=eval_model, results='asis', dev="cairo_pdf"}
...
```{r dunnet_stepdown, dev = "cairo_pdf", eval = eval_model, results = "asis"}
...
```{r dunnet_sandwich, dev = "cairo_pdf", eval = eval_model, results = "asis"}
...
```{r DunnetStepdownSandwich, dev= "cairo_pdf", eval=eval_model, results='asis'}
