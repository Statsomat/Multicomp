...
library(car) # type III Anova tests
...
# Save copies for other purposes 
  df_code <- df
  df2 <- df
...
cat(modelsplit[2])
cat("\\newline",fill=TRUE)
cat("You are interested in pairwise comparisons to the reference level:", fill=TRUE)
cat(modelsplit[3])
```
...
if (nrow(as.data.frame(df_factorized[complete.cases(df_factorized),])) < 10){
...
tryCatch({
  ...
  stop(safeError("Error: The model function could not be fitted to the data."))
...
number= fn1
# create table
cat("## Detailed Influence of Factors (Linear Model Parameter Estimates) \n", fill=TRUE)
...
```{r type3, dev="cairo_pdf", eval=eval, results='asis'}
# Type III tests 
...
if(df_table[1,'P.value']<0.05){
  df_table[1,'Interpretation (5% error)']='Intercept significantly different from zero.'
}else{df_table[1,'Interpretation (5% error)']='Intercept not significantly different from zero.'}

#make entries dependent on p-value
for(i in 2:nrow(df_table)){
  ifelse(df_table[i,'P.value']<=0.05,
         if(str_detect(rownames(df_table)[i], ":")){
      df_table[i,'Interpretation (5% error)'] <- paste0("Interaction ", rownames(df_table)[i], "statistically significant.")
         }else{
         df_table[i,'Interpretation (5% error)'] <- paste0("There exist significant differences between the levels of factor ", rownames(df_table)[i], ".")},
         if(str_detect(rownames(df_table)[i], ":")){
      df_table[i,'Interpretation (5% error)'] <- paste0("Interaction ", rownames(df_table)[i], "is not statistically significant.")
         }else{
         df_table[i,'Interpretation (5% error)'] <- paste0("There exist significant differences between the levels of factor ", rownames(df_table)[i], ".")
         }
      )
...
cat("## Total Influence of Factors (ANOVA Type III) \n", fill=TRUE)
options(knitr.kable.NA = '')
x1 <- knitr::kable(df_table, digits=2, escape = T, linesep = '', caption=cat(" ", fill=TRUE), longtable = T)
kable_styling(x1, position = "center", full_width = FALSE, latex_options = c("HOLD_position","repeat_header"))
```

```{r GoodnessOfFit, dev="cairo_pdf", eval = eval, results='asis'}
...
GoF <- data.frame('Values'=c('Multiple R-Squared'=sum.lmfit$r.squared,
                             ...
                             'F-statistic'=a$`F value`[2],'P-value'=a$`Pr(>F)`[2]))

GoF$Explanation <- c("Fraction of variance explained by the model.", 
                     "Adjusted R-Squared by penalizing higher p.", 
...
GoF$Interpretation[3] <-"Note the P-value to assess significance."

if (GoF$Values[4] < 0.05) GoF$Interpretation[4] <- "Significance to the 5% error. The model is better than the only-intercept model." else  GoF$Interpretation[4] <- "No significance to the 5% error. The model is better than the only-intercept model."
...
options(knitr.kable.NA = '')
x1 <- knitr::kable(GoF, digits=2, escape = T, linesep = '', caption=cat(" ", fill=TRUE), longtable = T)
kable_styling(x1, position = "center", full_width = FALSE, latex_options = c("HOLD_position","repeat_header"))

```

```{r diagnosticplots, dev="cairo_pdf", eval=eval, results="asis"}
...
```{r def_control_homog, eval=eval}
if(length(df_cat_factor)==1L) {
  oneway <- TRUE
}
```


```{r bftest1, dev="cairo_pdf", eval = eval,  results="asis"}
cat("\\pagebreak ", fill=TRUE)
# Homogeneity of Variances, Brown and Forsythe Test
cat("##  Homogeneity of Variances", fill=TRUE)
alphahomog <- 0.1
```

```{r bftest2, dev="cairo_pdf", eval = oneway, results="asis"}
# Title
cat("###  Brown and Forsyth Test of Homogeneity of Variances", fill=TRUE)
```


```{r bftest3, dev="cairo_pdf", eval = oneway}
...
if (bf$p.value <= alphahomog) {
   cat("Warning: Group variances significantly heterogeneous at 10% error! The Analysis of Variance may not be valid. ", fill=TRUE)
} else {
   cat("Interpretation: We assume that the group variances are homogeneous, therefore the Analysis of Variance results are valid. ", fill=TRUE)
   homogtest <- TRUE
}
```

```{r hovplot, dev="cairo_pdf", eval=oneway, results="asis", fig.width=8, fig.cap="Homogeneity of Variance Plot"}
# Brown and Forsythe Plot
cat("### Brown and Forsyth Plot", fill=TRUE)

## Plot for Brown and Forsythe 
...
```{r levene, dev = "cairo_pdf", eval = eval, results="asis"}
cat("### Levene Test of Homogeneity of Variances ", fill=TRUE)
```

```{r levene2, dev = "cairo_pdf", eval = eval}
levene.test(as.formula(modelfunction), data = df_factorized)
```

```{r levene3, dev="cairo_pdf", include=FALSE, results="asis"}
leveneobj <- levene.test(as.formula(modelfunction), data = df_factorized)
```

```{r levene4, dev = "cairo_pdf", eval = eval, results="asis"}
if (leveneobj$p.value <= alphahomog) {
   cat("Warning: Group variances significantly heterogeneous at 10% error! The Analysis of Variance may not be valid. ", fill=TRUE)
   homogtest <- FALSE
} else {
   cat("Interpretation: We assume that the group variances are homogeneous, therefore the Analysis of Variance results are valid. ", fill=TRUE)
...
```{r Dunnet, eval=eval, results='asis', dev="cairo_pdf"}
...
```{r dunnet_stepdown, dev = "cairo_pdf", eval = eval, results = "asis"}
...
conf_glht <- rownames_to_column(conf_glht, var="Null Hypothesis") %>% head
opts <- options(knitr.kable.NA = "")
x <- knitr::kable(conf_glht, digits=2, escape = F, linesep = '', caption="Simoultaneous Confidence Intervals: Dunnet Contrasts", longtable = T)
footnote(x, number = number_2) %>% 
  kable_styling(x, position = "center", full_width = FALSE, latex_options = c("HOLD_position","repeat_header"))
```
...
```{r dunnet_sandwich, dev = "cairo_pdf", eval = eval, results = "asis"}
...
for (i in 1:nrow(sandw.df)) {
  if(sandw.df[i,]$P.value <= 0.05){
    sandw.df[i,]$sig <- paste0("Significant. Level ", lev[i+1], " of factor ", key, " is significantly ", side, " than ", factorlevel, footnote_marker_number(4, "latex"))
  }else{ 
    sandw.df[i,]$sig <- paste0("Not Significant. Level ", factorlevel, " of factor ", key , " is ", side, " than " , lev[i+1], footnote_marker_number(3, "latex"))}
}
...
x1 <- knitr::kable(sandw.df , digits=2, escape = F, linesep = '', caption=cat(" ", fill=TRUE), longtable = T)
...
#plot(output_sandwich, main = "KI Sandwich", xlab = "x1")  # ylim = c(min(df$x1), max(df$x1)),
```
...
```{r DunnetStepdownSandwich, dev= "cairo_pdf", eval=eval, results='asis'}
cat("\n## Dunnet Step-Down Sandwich \n", fill=TRUE)
...
names(mylist) <- key
cat("\\small ", fill=TRUE)
output_sw_sd <- glht(lmfit, linfct = mcp2(mylist), alternative = "less", vcov = sandwich)
...
for (i in 1:nrow(dunnet_sw_sd)) {
  if(dunnet_sw_sd[i,]$P.value <= 0.05){
    dunnet_sw_sd[i,]$sig <- paste0("Significant. Level ", lev[i+1], " of factor ", key, " is significantly ", side, " than ", factorlevel, footnote_marker_number(3, "latex"))
  }else{ 
    dunnet_sw_sd[i,]$sig <- paste0("Not Significant. Level ", factorlevel, " of factor ", key , " is ", side, " than " , lev[i+1], footnote_marker_number(2, "latex"))}
}
# footnote significance codes
...
#Output table
opts <- options(knitr.kable.NA = "")
x <- knitr::kable(dunnet_sw_sd, digits=2, escape = F, linesep = '', caption=cat(" ", fill=TRUE), longtable = T)
footnote(x, number = number) %>%
  kable_styling(x, position = "center", full_width = FALSE, latex_options = c("HOLD_position","repeat_header"))
...
#simoultanous confidence intervals 
confmcp <- confint(output_sw_sd, level = 0.95) 
conf_glht <- as.data.frame(confmcp$confint)
colnames(conf_glht) <- c("Value", "Lower bound", "Upper bound")
rownames(conf_glht) <- paste(rownames(conf_glht),symb,"0")
conf_glht$sig <- NA
...
conf_glht <- rownames_to_column(conf_glht, var="Null Hypothesis") %>% head
opts <- options(knitr.kable.NA = "")
cat("\\begin{center} Simoultaneous Confidence Intervals: Dunnet Contrasts step-down Sandwich \\end{center}", fill=TRUE)
cat(paste("Simultaneous Confidence Intervals which include the true value of the difference between the reference level ", factorlevel, " and the other levels of ", key), fill=TRUE)

x1 <- knitr::kable(conf_glht, digits=2, escape = F, linesep = '', caption=cat(" ", fill=TRUE), longtable = T)
footnote(x1, number = number_2) %>% 
  kable_styling(x1, position = "center", full_width = FALSE, latex_options = c("HOLD_position","repeat_header"))
```
