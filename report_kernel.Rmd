...
#sandw.df <- data.frame("Hypothesis"=paste0(rownames(as.data.frame(sum.sandw$test$coefficients))," >= 0"), "Variable"=as.numeric(sum.sandw$test$coefficients),"sigma"=as.numeric(sum.sandw$test$sigma), "t-Test"=as.numeric(sum.sandw$test$tstat), "Pvalues"=as.numeric(sum.sandw$test$pvalues))
...
names(sandw.df)[5] <- paste0(names(sandw.df)[5], footnote_marker_number(1, "latex"))
...
names(sandw.df)[2] <- paste0(names(sandw.df)[2], footnote_marker_number(2, "latex"))

fn1 <- "Note: The sandwich tester was used to calculate this column."
...
cat("##Pairwise Comparisons Of Expression Levels \n", fill=TRUE)
...
#sandw.ki.df <- data.frame("Value"=as.numeric(sandw.ki$confint[,'Estimate']), "Lower bound"=as.numeric(sandw.ki$confint[,'lwr']),"Upper bound"=as.numeric(sandw.ki$confint[,'upr']))
