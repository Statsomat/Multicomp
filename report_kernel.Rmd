...
df_factor <- cbind(df_factor, df[which(sapply(df, is.factor) == 1L)])
...
if(length(df_cat_factor)==0 ){
  df_factorized <- as.data.frame(df_cont)
}else if( length(df_cont) == 0){
  df_factorized <- as.data.frame(df_cat_factor)
}else{
...
cat("\n# Descriptive Plots \n", fill=TRUE)
cat("\\small ", fill=TRUE)
cat("## Histogram and Boxplot for dependent Variable \n", fill=TRUE)

#Histogram and Boxplot for IV
...
# Boxplot for categorical DV
if(!(c(lhs(modelfunction))%in%c(colnames(df_cat)))){
  if(length(df_factor) != 0){
    cat("## Boxplot for categorical independent Variables \n", fill=TRUE)
...
if(length(df_cont)!=0 ){
  ...
  if (ncol(df_cont_iv)>0) {
    cat("## Scatterplot for numerical independent Variables \n", fill=TRUE)
    if (ncol(df_cont_iv)==1) {
      plot(x = c(1:nrow(df_cont_iv)) ,y=df_cont_iv[,1], col = "#2fa42d", main=paste("Scatterplot of", colnames(df_cont_iv))
           , xlab = "x", ylab = colnames(df_cont_iv))
