...
#Warning message:
#In check_reading(col) : NAs introduced by coercion
...
# Model information
# model funktion, also outputvariable~input1+input2+...
modelfunction <- modelsplit[1]
# welche variable wird als Referenzvariable verwendet
factor_index <- which(colnames(df_factorized) == modelsplit[2])

## New reference for the comparison 
# Reorder Levels of Factor: der Datensatz wird nach den Levels des Faktors (der als 
# Referenz verwendet wird) umsortiert.
...
# sapply return a Vektor of the same length as df_factorized. Jedes Element enthält das Ergebnis
# der funktion function(col), also wird die default contrast-matrix überschrieben und anstelle  
# dessen wird die Funktion contr.sum verwendet um die contrastmatrix festzulegen.

#df_factorized <- sapply(df_factorized, function(col) if (is.factor(col)) contrasts(col) <- contr.sum )
for (i in 1:ncol(df_factorized)) {
  if (is.factor(df_factorized[,i])==T) contrasts(df_factorized[,i]) <- contr.sum
}
#options(contrasts=rep("contr.sum",2))
#contrasts(df_factorized$blanket)
