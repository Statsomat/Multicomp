...
#cat("\\small ", fill=TRUE)
cat("Testing several null hypotheses at the same time at the significance level alpha increases the probability that at least one Type I error (false positive) will be made as the number of null hypotheses increases. This multiplicity problem is considered in multiple comparisons. In this case, the type I error is not controlled in every test for a null hypothesis, but the probability of at least one type I error is limited across all simultaneous tests. Using the adjusted p-value, the individual null hypotheses can be compared with the significance level alpha. Without multiple adjustment, reproducibility cannot be established.",fill=TRUE)
