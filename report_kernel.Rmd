...
# Split the text area containing the model information 
model <- params$model
...
if (bf$p.value <= alphahomog) {
   cat("Warning: Group variances significantly heterogeneous at 1% error! The Analysis of Variance may not be valid. ", fill=TRUE)
} else {
...
lev = leveneTest(as.formula(modelfunction), data = df_factorized)
lev2 <- as.data.frame(lev)
knitr::kable(lev2 , digits=2, escape = F, linesep = '', caption=cat(" ", fill=TRUE), longtable = T) %>%
  kable_styling(position = "center", full_width = FALSE, latex_options = c("HOLD_position","repeat_header"))

if(lev$`Pr(>F)`[1] <= alphahomog) {
     cat("Warning: Group variances significantly heterogeneous at 1% error! The Analysis of Variance may not be valid. ", fill=TRUE) 
     homogtest <- FALSE
} else {
   cat("Interpretation: We assume that the group variances are homogeneous, therefore the Analysis of Variance results are valid. ", fill=TRUE)
