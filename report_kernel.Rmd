...
# Interaction Plot
if(ncol(df_cat)>=2){
  if(grepl("[*]", modelfunction)==TRUE){ # plot only if interaction is in modelfunction
    ...
    if(grepl("[+]", modelfunction)==TRUE){
      ivs <- strsplit(as.character(rhs(modelfunction)), split="[+]")[[1]]
    }else{ ivs=format(rhs(modelfunction)) }
    ...
    for (i in 1:length(interactions)) {
      ...
      interaction.plot(df_cat[,grep(colnames(df_cat), pattern=inters[1])], df_cat[,grep(colnames(df_cat), pattern=inters[2])], x, xlab= inters[1], trace.label = inters[2], ylab=paste("mean of", lhs(modelfunction)), col = "#2fa42d",main=paste("Interaction Plot of ", inters[1], " and ", inters[2]))
        cat("\n \\newline \n ", fill=TRUE)
    }
  } 
}
...
#Output table
opts <- options(knitr.kable.NA = "")
cat("\\center Multiple Comparison: Dunnet Contrasts \\center", fill=TRUE)
...
opts <- options(knitr.kable.NA = "")
cat("\\center Simoultaneous Confidence Intervals: Dunnet Contrasts \\center",fill=TRUE)
