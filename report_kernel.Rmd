...
number= fn1
# create table
cat("## Effect of the separate expressions of the given variables (Parameter Estimates) \n", fill=TRUE)
...
\pagebreak
```{r, dev="cairo_pdf", eval=eval, results="asis", fig.width=5, fig.height=3, fig.cap="Homogeneity of Variance Plot", include=TRUE}
# Diagnostic Checks 
## Homogeneity of Variances 
cat("# Homogeneity of Variances", fill=TRUE)
oneway <- FALSE
alpha <- 0.05
if(dim(df_factorized)[2]==2) {oneway <- TRUE}
if (oneway == TRUE) {
  hovPlot(as.formula(modelfunction), data=df_factorized)
}
if (oneway == TRUE) {
  cat("\n \\newline \n Homogeneity of Variances Plot: ", fill=FALSE)
  cat("The observed data are represented by y. In the fist of the three plots, there are boxplots of each level of the independent variable. \n In the second plot y is subtracted by the median of the observed data, such that each median takes the value 0. \n  In the third plot the absolute deviations from the median to y are shown. The Brown and Forsyth test statistic is the F statistic resulting from an ordinary one-way analysis of variance on the data points in the third panel. \n", fill=TRUE)
  #detach("package:formula.tools", unload=TRUE)
  # bf <- bf.test(as.formula(modelfunction), data=df_factorized, verbose=TRUE)
  # if (bf$p.value <= bf$alpha) {
  #   cat("Homogeneity of Variances is not given.", fill=TRUE)
  # } else {cat("Homogeneity of Variances is given.", fill=TRUE)}
  cat("Levens Test for HOV: \n ", fill=TRUE)
  lev = leveneTest(as.formula(modelfunction), data=df_factorized)
  x6 <- as.data.frame(lev)
} else {
  cat("Levens Test for HOV: \n", fill=TRUE)
  inter_act= as.formula(modelsplit)
  op(rhs(inter_act))="*" # interaction for Levene Test
  lev=leveneTest(inter_act, data=df_factorized)
  x6 <- as.data.frame(lev)
}
x5 <- knitr::kable(x6 , digits=2, escape = F, linesep = '',
                   caption=cat(" ", fill=TRUE), longtable = T)
x5%>%
kable_styling(x5, position = "center", full_width = FALSE, latex_options = c("HOLD_position","repeat_header"))
if(lev$`Pr(>F)`[1]<= alpha) {
    cat("Homogeneity of Variances is not given.", fill=TRUE) 
    } else {cat("Homogeneity of Variances is given.", fill=TRUE)}
## Normality
